...
```{r}
data(UndergroundRR)
u_rr_data_coordinates <- undergroundRR |> geocode(city = City, state = State, method='osm', lat = latitude, long = longitude)
head(u_rr_data_coordinates)
```
...
library(dplyr)
bwv_data <- BostonWomenVoters |>
mutate(address = paste(Street.Number.on.April.1, Street.of.Residence.on.April.1, Unit, "Boston", sep=" "))

bwv_data_coordinates <- bwv_data |> slice(1:25) |> 
geocode(address = address, method='osm', lat = latitude, long = longitude)

head(bwv_data_coordinates)
```
...
> The package allows domestic access to county, state, and USA maps. It also allows mapping France, Italy, New Zearland, and a more broad world mapping. 
...
> The data frame gives longitude and latitudes  for each region in every state based off of group and order. 
...
```{r}
continental_gg <- gayguides |> filter(state != "HI")
ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = continental_gg, mapping = aes(x = lon, y = lat), color = "orange", size = 3) + coord_fixed(1.3)
...
ggplot() + 
  geom_map(data = sc_map, map = sc_map, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = sc_gg, mapping = aes(x = lon, y = lat), color = "orange", size = 3) + coord_fixed(1.3)
```
...
ggplot() + 
  geom_map(data = ma_map, map = ma_map, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = bwv_data_coordinates, mapping = aes(x = longitude, y = latitude), color = "orange", size = 3) +coord_fixed(1.3)

ma_county <- map_data("county", region = "massachusetts")
ggplot() + 
  geom_map(data = ma_county, map = ma_county, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = bwv_data_coordinates, mapping = aes(x = longitude, y = latitude), color = "blue", size = 3) +coord_fixed(1.3)
```
...
nhgis2 <- ipums_shape_full_join(nhgis_data, nhgis_shp, by = "GISJOIN")
...
ggplot(data = nhgis2, aes(fill = AZF001)) +
  geom_sf() 
...
> Filtering out Hawaii and Alaska allows for a representation of the continental USA. You may want to do this because you are only focusing on the continental US, or you want a map that looks more clean. If you do not include both these other states, you cannot say it represents the entiriety of the union. If they both remain filtered out, then the map only represents the continental US and that is what should be interpreted. 
...
```{r}
nhgis <- nhgis |> mutate(total_male_pop = AZF001 + AZF003)
```

(@) Now, use the total_male_pop variable and create a variable for the the percentage of foreign born males.
```{r}
nhgis <- nhgis |> mutate(foreign_male_percentage = AZF003 / total_male_pop)
```
...
> They are continous and binned color scales. It is the defealt color scales ggplot2 uses when continous data calues are mapped into either color or fill and they are equivalent scale funtions where color bins are assigned continous values instead of using a continous color spectrum. Arguments consist of name, breaks, n.breaks, amongst oithers. The code cna use the scale to five a palette directly or default colors that are encoded into the theme. You can set your default color theme as well. 

Now create the map: 
```{r}
ggplot(data = nhgis, aes(fill = foreign_male_percentage)) +
  geom_sf() + scale_fill_continuous(name = "Percentage of Native Bron Males", palette = "Spectral", aesthetics = "fill", guide = "colorbar", labels = scales::percent)
```
...
leaflet(data=continental_gg) |> addTiles() |>
 addMarkers(~lon, ~lat, popup = paste("The",continental_gg$title, "located on", continental_gg$streetaddress, ",", continental_gg$city, continental_gg$state, "was a", continental_gg$type, "in", continental_gg$Year, sep=" "))

bars_gg <-  continental_gg |> filter(type == "Restaurant")
leaflet(data=bars_gg) |> addTiles() |>
 addMarkers(~lon, ~lat, popup = paste("The",bars_gg$title, "located on", bars_gg$streetaddress, ",", bars_gg$city, bars_gg$state, "was a", bars_gg$type, "in", bars_gg$Year, sep=" "))
```
...
> I decided to map two different things. The first map looks at all locations in gayguides within the continental USA. The other lines map only restuarants within the continental USA. The first map was made to see the overall look and appearance of the greater gay guides data. The second map was to look at the data set in specifics. Leaflet beats out ggplot in terms of allowing greater specifics and interactivity. It is great for a website or something in which the user can interact with the map. In comparison, ggplot seems to be the better mapping device to use for papers and books because of the general maps it creates. Maps in a book cannot be clicked on, so leaflet becomes less viable. 
...
leaflet(data=parks) |> addTiles() |>
 addMarkers(~lon, ~lat, popup = paste("The",parks$park,"National Park,", "founded in", parks$state_region_province, ",", parks$country, "in", parks$year, "consists of", parks$total_sq_kilometers, "square kilometers and is", parks$distance_from_closest_city, "kilometers from the closest city", sep=" "))

world <- map_data("world")

ggplot() + 
  geom_map(data = world, map = world, aes(long, lat, map_id = region), fill = "lightgray", color = "black")+
  geom_point(data = parks, mapping = aes(x = lon, y = lat), color = "blue", size = 2) + 
  coord_fixed(1.3) +
