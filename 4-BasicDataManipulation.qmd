...
>Used the select funtion to select only the city and state columns from the gayguides dataset to make the locations dataset. 
...
```{r}
locations_minus <- gayguides |> select(-city, -state)
...
```{r}
greenville_gayguides <- gayguides |> filter(state == "SC" & city == "Greenville")
gayguides |> filter(state == "SC" & city == "Greenville")
```

(@) How about every location between 1975 and 1980? 

```{r}
location_gayguides <- gayguides |> filter(Year >= 1975 & Year <= 1980)
```
...
```{r}
greenville_year_gayguides <- gayguides |> filter(state == "SC" & city == "Greenville") |> filter(Year >= 1975 & Year <= 1980)
```

(@) Can you find all locations in 1975 except for New York and San Francisco? 
```{r}
no_ny_sf_gayguides <- gayguides |> filter(Year == 1975 & city != "New York") |> filter(city != "San Francisco")
```
...
BostonWomenVoters_birth_year <- BostonWomenVoters |> mutate(birth_year = 1920 - Age)
```
...
```{r}
s_gayguides <- gayguides |> mutate(location = paste(city, state, sep=", "))
```
...
```{r}
title_gayguides <- gayguides |> mutate(city_title = str_to_title(city)) 
```
...
```{r}
dancing_gayguides <- gayguides |> mutate(dancing = grepl("(D)", gayguides$amenityfeatures))
```

(@) In the Boston Women Voters data, occupation names may have extra whitespace. Use `str_trim()` to clean the occupation column.
```{r}
BostonWomenVoters |> mutate(occupations = str_trim(BostonWomenVoters$occupation))
str_trim(BostonWomenVoters$occupation)

BostonWomenVoters |> mutate(Occupation = str_trim(BostonWomenVoters$Occupation))
str_trim(BostonWomenVoters$Occupation)
????
```
...
```{r}
state_pop <- statepopulations |> select(STATE, X1850) |> arrange(desc(X1850)) 
head(state_pop, 1)
## New York 
```
...
```{r}
BostonWomenVoters |> drop_na(Ward) |> nrow()
##10660
```

(@) Why might ward information be missing for some voters? What does this tell us about the historical record?

>There could have been a failure to fully complete the vote by a worker or voter. The person who made the dataset could have missed inputing the data. Without the data on ward, most of all the data will be NA and as such, a lot of the record may not be useable in comparison to other entries. 
...
```{r}
clean_BostonWomentVoters <- BostonWomenVoters |> drop_na(Age, Ward) |> nrow()
##10609
```
...
```{r}
n_a_BostonWomentVoters <- BostonWomenVoters |> mutate(ward_missing = is.na(BostonWomenVoters$Ward))
is.na(BostonWomenVoters$Ward)
```
...
# Convert to factor
gayguides <- gayguides |> mutate(state_factor = as.factor(state))
...
```{r}
gayguides <- gayguides |> mutate(type = as.factor(type))
class(gayguides$type)
levels(gayguides$type)
##235
```

(@) In the Boston Women Voters data, convert Occupation to a factor. What are the levels?
```{r}
BostonWomenVoters <- BostonWomenVoters |> mutate(occupation = as.factor(occupation)) 
BostonWomenVoters <- BostonWomenVoters |> mutate(Occupation = as.factor(Occupation))
