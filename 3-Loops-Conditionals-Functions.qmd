...
} else { print("Enrollment is closed")
  }
```
...
```{r}
presidents <- c("Washington", "Jefferson", "Adams", "Obama", "Clinton", "Bush")
...
presidents <- c("Washington", "Jefferson", "Adams", "Obama", "Clinton", "Bush")
if (length(presidents) >= 5) {
  print("The number of presidents in the list is more than 5") } else {
  print("The number of presidents in the list is less than 5")
}
```
...
```{r}
if(1990 < max(gayguides$Year)){
  print(TRUE)} else {
    print(FALSE)
}
## or 
if(1990 < max(gayguides$Year)){
  print("The GayGuides dataset does go past the year 1990")} else {
    print("The GayGuides dataset does not go past the year 1990")
}
```
...
> Well first off I knew we had to find whether or not a year in the gayguides dataset went past the year 1990. To do this I used the max() function on the subsetted dataset. Gayguides does not go beyond the year 1990 because the most recent year is 1985. 
...
> the length() function sets or returns the items in vectors and factors, and of any other R object for which a method has been defined. It is used for presidents so that each indivudal observation in the vector is returned seperately. 
...
```{r}
classes <- c("I am enrolled in HIST 8500:Digital Methods for History I", "I am enrolled in HIST 8540: Local History and Digital Archives", "I am enrolled in Thesis Hours")
...
The beginning function skips any of the numbers less than 3. The next function skips any numbers greater than or equal to five. Due to the conditions only 3 and 4 fit the criteria and are printed. 
...
```{r}
historyStudents("Chase McNeill", "History 8500")
```
...
```{r}
greet <- function(name) {
  statement <- paste("Hello, ", name, "!", sep="")
  statement
}
greet("Chase McNeill")
```
...
```{r}
string_of_characters <- function(characters) {
  string <- paste("There are", nchar(characters), "characters in that string.", sep=" ")
  string
}
string_of_characters("Hello, Chase McNeill!")
```

(@) Reflect on the process of building the above function. How did you go about figuring this out? 

> I knew I had to make a function that would count the number of characters in a string. I built most of my function based off the prior example, only except I would have to replace "name" with a function that would count the characters of my input. That is where nchar() comes in and to make it work I added "characters" inside of the function. Then all I had to do was see if the function worked. 
...
> GeorgeWashington is coded as a new character variable. If you want to see the letter grade you would just have to type in GeorgeWashington into your console, "C", or look at your environment. 
...
```{r}
state_year <- function(state, year) {
  data(gayguides)
  for (i in 1:length(gayguides$state)) {
    if (gayguides$state[i] == state && gayguides$Year[i] == year) {
     print(paste("Found a location in", state, "called", gayguides$title[i]), sep=" ")
    } else {
      next 
    } 
  }
...
bwv.occupation.result <- bwv.occupation("Housewife")
...
```{r}
count_by_state_year <- function(abstate, abyear) {
  data(gayguides)
  locations_in_state_year <- 0
  for (i in 1:nrow(gayguides)) {
    if (gayguides$state[i] == abstate && gayguides$Year[i] == abyear) {
    locations_in_state_year <-locations_in_state_year + 1
    } else {
      next 
    } 
  }
  print(paste("There are", locations_in_state_year, "locations in", abstate, sep=" "))
}
count_by_state_year("NC", 1983)
count_by_state_year("SC", 1975)
```
...
> In the prior question to find the amount of locations for a given year and state, it required using all three of loops, conditionals, and functions. Firstly, the function works as a set of code that performs a task you want to complete over and over again. To formulate more complex functions you need to use loops and conditionals. The conditionals and choices cycle through datasets to help find what you are looking for, such as using the function if(). As for loops they look at each item in the vector, and looks at if a specific action is called once and continues to update the value everytime. My example creates the function count_by_state_year by looking at specific states and years. If the year and state is found it updates the loop and continues until there is nothing left to check. Then it pastes the result of the code. The function is then left to be used over and over again as I see fit. 