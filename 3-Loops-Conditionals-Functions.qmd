...
```{r}
if(1990 < gayguides$Year) {
  print(TRUE)} else {
    print(FALSE)
}
### Maybe use this as the question one idk. 
...
for (i in 1:length(grepl("Bar", gayguides$titles, ignore.case = FALSE))) {
  print(gayguides$title[i])
}

## for (i in 1:length(gayguides$title)) {
## print(grepl("Bar", gayguides$title[], ignore.case = TRUE)) }
## 
## Messed up and ran this line. Had to kill terminal. 
for (i in 1:length(gayguides$title)) { if grepl(("Bar", gayguides$title, ignore.case = TRUE)){print(gayguides$title[i])} else FALSE 
}
grepl("Bar", gayguides$title, ignore.case = TRUE)
