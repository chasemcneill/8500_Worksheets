...
> This function returns all the values in the vector x besides the numbers first and third values. 
...
> If an item in the vector is greater than 8, TRUE, it will list it in the output. If not, it results in FALSE and the item is not returned in the output. 
...
The funtion creates a new vector, y, which has set names for the first four items in the original vector of x. The other three items are NA because the function only goes to the letter d, because it specifies only four letters. Lastly, the second line is looking for the letter d, which outputs the fourth item, 8.35. 
#### Matrices
...
> In the first example it is pulling all of the data in columns two and three. In the second example it is pulling all the data in rows one to three.

(@) Can you find just the city and state for the first 5 entries? 
```{r}
head(gayguides[8:9], 5)
```
...
```
Messed up the order of column and row. 
...
charlestonManumissions[236:241, 3:4]
```

> I wanted to find the owner and motive of the manumissions from rows 236 to 241. 
...
The code is looking at the data frame "gayguides" which through the operator $ looks at the specific column or varibale "city" within the data frame. The last section [100:110] tells the code to look for just the city in rows 100 to 110. 

(@) Can you find all the titles of locations in South Carolina? (Hint: You'll need to use both `$` and `[]`)
```{r}
sc <- gayguides[gayguides$state == "SC",] 
sc[, 3]
...
california <- gayguides[gayguides$state == "CA",] 
1970 <= gayguides$Year & gayguides$Year < 1980 
california_1970s <- california[1970 <= california$Year & california$Year < 1980,]
unique(california_1970s[, 3])

## or

other_ca_1970s <- gayguides[gayguides$state =="CA" & 1970 <= gayguides$Year & gayguides$Year < 1980, ]
```
...
gg_1975 <- gayguides[gayguides$Year == 1975, ]
tf_girls_women <- grepl("(G)", gg_1975$amenityfeatures, ignore.case = TRUE)
tf_girls_women
## or
girls_women <- gg_1975[grepl("(G)", gg_1975$amenityfeatures, ignore.case = TRUE), ]
nrow(girls_women)
## 253 locations specifically catered to women in 1975. 
```
...
```{r}
over_50 <- BostonWomenVoters[50 < BostonWomenVoters$Age, ]
nrow(over_50)
## 2408
under_50 <- BostonWomenVoters[50 > BostonWomenVoters$Age, ]
nrow(under_50)
## 8119
In terms of the age distribution of women voters in Boston, there is a massive majority who fall under the age of 50 with 8119 voters. Those over the age of 50 comprised of only 2408 voters, a lot less than their younger counterparts. 
```
...
```{r}
main_street <- grepl("Main Street", BostonWomenVoters$Street.of.Residence.on.April.1, ignore.case = FALSE)
main_street_dataset <- BostonWomenVoters[grepl("Main Street", BostonWomenVoters$Street.of.Residence.on.April.1, ignore.case = FALSE), ]
main_street
BostonWomenVoters$Street.of.Residence.on.April.1 == "Main Street"
main_street_dataset[ , ]
```
...
age_order <- BostonWomenVoters[order(BostonWomenVoters$Age, decreasing = TRUE), ]
head(age_order, 5)
## The oldest two women work at home, next possibly at "home?"", fourth is a Housekeeper, and the fifth is a retired widow. 
