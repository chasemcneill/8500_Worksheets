...
```{r}
paste("Hello,", "my name is Chase.")
```

(@) Can you explain what happened in 2-3 sentences? 

> The function "paste()" combines both the strings "Hello," and "My name is Chase." The comma between the two works as an combining function that causes the function paste to combine the two and output "Hello, my name is Chase."
...
> Line 96 returns FALSE because 5 is not equal to 10. 
> Line 97 returns TRUE because 3 is less than 4. 
> Line 98 returns TRUE because 5 is not equal to 10. 
> Line 99 returns FALSE because 5 is equal to 5. 
> Line 100 returns TRUE because 3 is equal to at least one of 4 or 3.
> Line 101 returns TRUE because 3 is not equal to both 4 and 5. 
...
>The vector "my.numbers" consistes of 4 numerical items "2, 4, 6, 8". These numbers are each multiplied by 2 because of the function my.numbers * 2 does not specify an individual item. The line returns 4, 8, 12, and 16 respectively. 
...
>The vector my.numbers is again chosen to be multiplied by 2, except the third item has been specified. This means that the code ignores all items that are not equal to three, resulting in only 6 being multiplied by 2. The result of this line is 12. 

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
odd_nummbers <- c(1, 3, 5, 7, 9)
```
...
> floor() takes a single numeric argument (x) and returns a numeric vecrtor containing the largest integers not greater than the corresponding elements of (x). 
...
> Each line in z corresponds to the respective lines in x and y. This means for row one because x = 1 and y = 1, then the equation to find z is 1 * 2 + 1, which equals z's value of 3 in row 1. Each respective row follows the same logic in how z is coded. 
...
> gayguides is one data frame with 14 variables and 60698 observations. The dataset contains information transcribed from Bob Damron's Address Books from 1965 through 1985. 
...
> For the first prompt, head is how you get the first set of rows, so just place the 10 to gain past the default 6. The second question is asking for the last 6 rows of data, so use tail(), which is the opposite of head().
...
>This line of code returns only the observation of the variable "title" in the 500th[500] item of the dataset "gayguides." The $ is the operator that allows you single out a specific variable you want to look at in the dataset "gayguides".
...
colnames(charlestonManumissions)
nrow(charlestonManumissions)
str(charlestonManumissions)
head(charlestonManumissions)
charlestonManumissions$Page <- as.numeric(as.character(charlestonManumissions$Page, na.strings = "NA"))
min(charlestonManumissions$Page, na.rm = TRUE)
max(charlestonManumissions$Page, na.rm = TRUE)
mean(charlestonManumissions$Page, na.rm = TRUE)
median(charlestonManumissions$Page, na.rm = TRUE)
```

(@) What did you learn about the dataset? What can you tell me about it? 

> Charleston Manumissions is a dataset containing all of the manumissions filed in Charleston District from 1776-1800 in the Miscelleanous Records of the South Carolina Department of Archives and History. The dataset has 271 observations of 10 variables, consisting of things like date of the manumission and amount of pages. I changed the varibale "Page" to a numeric column so that I could use the variable easier for functions. The mean for "Page" is 242.36 and the median is 245. 
